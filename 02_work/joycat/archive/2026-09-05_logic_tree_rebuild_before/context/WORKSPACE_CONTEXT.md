...
KPI Tree và Metric Tree đã có bản làm việc. Định nghĩa bốn chiều, sáu cặp, coverage theo từng tổ hợp, audit schema, đường phân tích metric, drill-down và hợp đồng chuẩn bị ETL/report đã được deep-audit trong `03_outputs\joycat\KHUNG_PHAN_TICH_CONG_THUC_VA_MAPPING_JOYCAT.md/.mm`. Đây là reference model, không phải Logic Tree thuần. Logic Tree đúng nghĩa vẫn cần được rút ra theo một câu hỏi phân tích cụ thể sau review; việc hoàn thiện reference model không đồng nghĩa dataset đã đủ hoặc ETL/report production được phép triển khai.
...
| `03_outputs\joycat\KHUNG_PHAN_TICH_CONG_THUC_VA_MAPPING_JOYCAT.md` | Reference model: bốn chiều, sáu cặp, coverage, công thức, funnel/journey, ETL contract và gaps | Bộ đọc chi tiết cho Joycat; không gọi là Logic Tree |
| `03_outputs\joycat\KHUNG_PHAN_TICH_CONG_THUC_VA_MAPPING_JOYCAT.mm` | Mindmap cùng nhánh chính với reference model | Dùng review trực quan; cần mở thử trên Lark/MindManager |
...
| Logic Tree Joycat | Chưa tách thành artefact đúng nghĩa | Duy + Codex; cậu Sinh review | Cần khóa câu hỏi phân tích rồi rẽ câu hỏi → giả thuyết → bằng chứng → quyết định | Dựng sau khi review reference model |
| Bốn chiều, sáu cặp và khung công thức | Có reference model đầy đủ source/mapping contract và coverage theo tổ hợp | `03_outputs\joycat\KHUNG_PHAN_TICH_CONG_THUC_VA_MAPPING_JOYCAT.md`; working audit trong `02_work\joycat\coverage_audit` | Ba cặp có platform bị chặn; ba cặp còn lại mới human mapping/một phần; nguồn demo tháng 04 còn lệch preferred Campaign | Xin source/mapping còn thiếu và xử lý lineage trước ETL production |
...
Bước tiếp theo hiện tại: Duy/cậu Sinh review `WORKSPACE_CONTEXT.md` → `KHUNG_PHAN_TICH_CONG_THUC_VA_MAPPING_JOYCAT.md` → mindmap tương ứng, rồi phản hồi mapping/data gate và khóa câu hỏi cho Logic Tree. Chưa triển khai ETL/report hoặc kết luận hiệu quả Joycat.
