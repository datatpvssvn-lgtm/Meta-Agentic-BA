...
KPI Tree và Metric Tree đã có bản làm việc. Định nghĩa bốn chiều, sáu cặp, coverage, audit schema và đường phân tích đã được gộp vào `03_outputs\joycat\LOGIC_TREE.md/.mm`. Bộ này đang chờ Duy/cậu Sinh review; việc gộp xong không đồng nghĩa Logic đã được duyệt, dataset đã đủ cho mọi cặp hoặc ETL/report production được phép triển khai.
...
| `03_outputs\joycat\LOGIC_TREE.md` | Tài liệu hợp nhất: bốn chiều, sáu cặp, coverage, funnel/journey, ETL contract và gaps | Bộ đọc chính cho Joycat |
| `03_outputs\joycat\LOGIC_TREE.mm` | Mindmap cùng nhánh chính với tài liệu hợp nhất | Dùng review trực quan; cần mở thử trên Lark/MindManager |
...
| Logic Tree Joycat | Có bản hợp nhất v2.0; chưa được duyệt | Duy + cậu Sinh | Chưa được dùng như kết luận nguyên nhân | Review `03_outputs\joycat\LOGIC_TREE.md/.mm` |
| Bốn chiều và sáu cặp | Đã nằm cạnh định nghĩa, công thức, source và coverage trong Logic Tree hợp nhất | `03_outputs\joycat\LOGIC_TREE.md` | Ba cặp có platform bị chặn; ba cặp còn lại mới human mapping/một phần | Xin source/mapping còn thiếu trước ETL |
...
Bước tiếp theo hiện tại: Duy/cậu Sinh review bộ đọc chính `WORKSPACE_CONTEXT.md` → `LOGIC_TREE.md` → `LOGIC_TREE.mm`, kiểm tra mindmap trên Lark/MindManager và phản hồi các mapping/data gate. Chưa triển khai ETL/report hoặc kết luận hiệu quả Joycat.
