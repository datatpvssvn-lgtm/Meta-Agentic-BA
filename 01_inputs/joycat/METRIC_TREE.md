...
> Phạm vi: Viết công thức cho các KPI trong [KPI v3 hiện hành](Ad_Cost_GMV_all_platform%20v3.md); định nghĩa bốn chiều và coverage sáu cặp nằm tại `DATA_MAPPING_COVERAGE_JOYCAT.md`. Chưa phân tích nguyên nhân và chưa kết luận tỷ lệ 5–10%.
...
> Mỗi KPI trong [KPI v3 hiện hành](Ad_Cost_GMV_all_platform%20v3.md) được tính từ những đại lượng nào, các đại lượng đó lấy từ đâu và phải dừng phân rã tại điểm nào?
...
flowchart LR
    ...
    CPP --> CPP_SPEND["SUM Amount Spent của dòng có purchase indicator"]
    ...
    AC --> CMP["COST PER META-ATTRIBUTED PURCHASE — TOÀN SCOPE"]
    CMP --> CMP_SPEND["SUM Amount Spent của toàn scope"]
    CMP_SPEND --> CMP_SPEND_RAW["Amount spent VND<br/>Meta export trực tiếp — DỪNG"]
    CMP --> CMP_PURCHASE["SUM meta_purchases_attributed cùng attribution"]
    CMP_PURCHASE --> CMP_PURCHASE_RAW["Purchases<br/>Meta export trực tiếp — DỪNG"]
...
| Cost per Result — Purchase indicator | `SUM Amount Spent của dòng purchase indicator / SUM Results cùng indicator` | `Amount spent (VND)`; `Results`; `Result indicator` | VND/result theo indicator | Sẵn có có điều kiện | `Cost per results` trong đúng nhóm indicator |
| Cost per Meta-attributed Purchase — toàn scope | `SUM Amount Spent của toàn scope / SUM meta_purchases_attributed cùng scope và attribution` | `Amount spent (VND)`; cột nguồn `Purchases`; `Attribution setting` | VND/Meta-attributed purchase | Sẵn có một phần | Không dùng cột CPR của riêng purchase-indicator để thay thế |
| Cost per Business Order | `SUM Ads Cost cùng scope / SUM business_orders_eligible cùng scope` | Meta spend + business order source | VND/đơn business | Chưa sẵn có | Không có trong Meta export |
...
### Meta Purchase, Orders Created và Orders Dispatched

Ba field này là event Meta xuất trực tiếp nên Metric Tree dừng tại đó. Trong tài liệu chuẩn hóa, cột `Purchases` được gọi là `meta_purchases_attributed`. Không nối toán học:

```text
meta_purchases_attributed → Orders Created → Orders Dispatched
```
...
| business_orders_eligible | Không | To be updated từ business source; không thay bằng cột Purchases Meta |
...
| `meta_purchases_attributed` được tách khỏi `business_orders_eligible` | Đạt ở tầng định nghĩa; bridge business chưa sẵn có |
